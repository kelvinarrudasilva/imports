...
st.markdown(
    """
    <style>
      :root { --green:#228B22; --bg:#F5F5F5; --card:#DFF0D8; --muted:#555555; }
      .stApp { background-color: var(--bg); color: var(--green); }
      .title { color: var(--green); font-weight:700; font-size:22px; }
      .subtitle { color: var(--muted); font-size:12px; margin-bottom:12px; }
      .kpi { background: linear-gradient(90deg, #E6F1E0, #DFF0D8); padding:12px; border-radius:10px; text-align:center; }
      .kpi-value { color: var(--green); font-size:20px; font-weight:700; }
      .kpi-label { color:var(--muted); font-size:13px; }
      .stDataFrame table { background-color:#E6F1E0; color:#228B22; }
    </style>
    """,
...
with tab1:
    ...
    if v_prod in (vendas_f.columns if vendas_f is not None else []) and v_qtd in (vendas_f.columns if vendas_f is not None else []):
        ...
        if not top.empty:
            fig_top = px.bar(top, x="VAL_TOTAL", y=v_prod, orientation="h", text="QTDE_SOMADA", color="VAL_TOTAL", color_continuous_scale=["#228B22","#006400"])
            fig_top.update_traces(texttemplate='%{text:.0f} un', textposition='outside')
            fig_top.update_layout(plot_bgcolor="#F5F5F5", paper_bgcolor="#F5F5F5", font_color="#228B22", yaxis={'categoryorder':'total ascending'})
    ...
    if v_data in (vendas.columns if vendas is not None else []):
        ...
        if "_VAL_TOTAL" in tmp.columns:
            ...
            fig_vmes = px.bar(vendas_mes, x="_MES", y="VALOR", title="Vendas Mensais", color="VALOR", color_continuous_scale=["#228B22","#006400"])
            fig_vmes.update_layout(plot_bgcolor="#F5F5F5", paper_bgcolor="#F5F5F5", font_color="#228B22")
        ...
        if "_LUCRO" in tmp.columns:
            ...
            fig_luc.update_traces(line=dict(color="#228B22"))
            fig_luc.update_layout(plot_bgcolor="#F5F5F5", paper_bgcolor="#F5F5F5", font_color="#228B22")
