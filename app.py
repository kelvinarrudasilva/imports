...
with tabs[2]:
    ...
    <style>
        ...
        .btn-limpar:hover { filter: brightness(1.2); }
        ...
        .ctitle{ font-size:16px; font-weight:800; color:white; }
        .cmeta{ font-size:12px; color:#ccc; margin-top:4px; }
        .cprice{ color:#a78bfa; font-weight:900; font-size:16px; margin-top:6px; }
        .avatar{
    ...
    # BOTÃO LIMPAR TUDO
    # ==========================================================
    col_limpa, _ = st.columns([1,5])
    with col_limpa:
        if st.button("🧹 LIMPAR TUDO", help="Reseta todos os filtros", key="limpartudo"):
            for x in ["termo","ordenar","cols","getitem","ver_tudo",
                      "f_baixo","f_alto","f_vendidos","f_sem_venda","pagina"]:
                st.session_state[x] = None
            st.rerun()

    # ==========================================================
    # BOX DE BUSCA + CONTROLES
    # ==========================================================
    st.markdown("<div class='search-box'>", unsafe_allow_html=True)

    termo = st.text_input(
        "Buscar",
        st.session_state.get("termo",""),
        key="termo",
    ...
    with col1:
        ordenar = st.selectbox(
            "Ordenar por",
            ["Nome A–Z","Nome Z–A","Menor preço","Maior preço","Maior estoque","Menor estoque"],
            index=0,
            key="ordenar"
        )
    with col2:
    ...
    with col3:
        cols = st.selectbox("Colunas", [2,3,4], index=1, key="cols")
    with col4:
    ...
    with colf1:
        f_baixo = st.checkbox("⚠️ Baixo estoque", key="f_baixo")
    with colf2:
        f_alto = st.checkbox("📦 Alto estoque", key="f_alto")
    with colf3:
    ...
    elif ordenar == "Menor estoque":
        df = df.sort_values("EM ESTOQUE", ascending=True)
    ...
    if not ver_tudo:
        total_paginas = max(1, (total + itens_pag - 1)//itens_pag)
        ...
        inicio = (st.session_state["pagina"]-1)*itens_pag
    ...
    for _, r in df.iterrows():
        nome = r["PRODUTO"]
        estoque = r["EM ESTOQUE"]
        preco = f"R$ {r['Valor Venda Sugerido']:.2f}".replace(".",",")
        ...
        <div class='card'>
            ...
            <div>
                ...
                <div class='cmeta'>Estoque: {estoque}</div>
                <div class='cprice'>{preco}</div>
            </div>
        </div>
        """, unsafe_allow_html=True)
