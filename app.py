# app.py — Dashboard Loja Importados completo e final
...
def parse_money_series(serie): return serie.astype(str).map(lambda x: parse_money_value(x)).astype("float64")
def parse_int_series(serie):
    def to_int(x):
        ...
        s = str(x)
        s = re.sub(r"[^\d\-]", "", s)
        if s=="" or s=="-" or s.lower()=="nan": return pd.NA
...
def filtrar_mes(df, mes):
    ...
    if "MES_ANO" in df.columns: return df[df["MES_ANO"]==mes].copy()
    return df
...
def calcular_totais_vendas(df):
    ...
    total_vendido = (df["VALOR TOTAL"].fillna(0) if "VALOR TOTAL" in df.columns else df["VALOR VENDA"].fillna(0)*df["QTD"].fillna(0)).sum()
...
with tabs[1]:
    ...
    if vendas_filtradas.empty:
        ...
        top_val_display = formatar_valor_reais(top_val_display, ["VALOR_TOTAL"])
        st.dataframe(top_val_display, use_container_width=True)
...
with tabs[3]:
    ...
    if vendas_filtradas.empty:
        ...
        fig3 = px.bar(top_lucro, x="PRODUTO", y="LUCRO_TOTAL",
        ...
        top_lucro_display = top_lucro.copy()
        top_lucro_display = formatar_valor_reais(top_lucro_display, ["LUCRO_TOTAL"])
        st.dataframe(top_lucro_display, use_container_width=True)
